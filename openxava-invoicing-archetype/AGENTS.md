...
<controller name="Invoice">
    ...
    <action name="showYear" mode="detail"
        class="${package}.${artifactId}.actions.ShowYearAction"/>
</controller>
```
...
public class ShowYearAction extends ViewBaseAction {
    ...
    public void execute() throws Exception {
        ...
        if (year == null) {
            addError("year_not_set"); // Add year_not_set to i18n messages file
        } else {
            addMessage("year_message", year); // Add year_message to i18n messages file
        }
    }
}
```
