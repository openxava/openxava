...
```xml
<controller name="Master">
    <extends controller="Typical"/>
    <action name="showTotal" mode="detail"
        class="${package}.${artifactId}.actions.ShowTotalAction"/>
</controller>
```
...
public class ShowTotalAction extends ViewBaseAction {
    ...
    public void execute() throws Exception {
        ...
        if (total == null) {
            addError("total_not_set"); // Add total_not_set to i18n messages file
        } else {
            addMessage("total_message", total); // Add total_message to i18n messages file
        }
    }
}
```
