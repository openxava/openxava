...
public class Emails {
    ...
    public static void sendInBackground(String toEmail, String subject, String content) {  
    	// tmp new Thread(new BackgroundSender(XavaPreferences.getInstance().getSMTPUserID(), toEmail, subject, content)).start();
    	new Thread(new BackgroundSender(XavaPreferences.getInstance().getSMTPFromEmail(), toEmail, subject, content)).start(); // tmp
    }
