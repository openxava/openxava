...
public class MetaView extends MetaElement implements Cloneable {
	...
	// WARNING!: If you add properties you must see if is needed to make a clon of
	// them
	...
	public void addMetaViewProperty(MetaProperty metaProperty) throws XavaException {
		if (metaViewProperties == null)
			metaViewProperties = new HashMap();
	...
	public void addMetaViewReference(MetaReferenceView metaReferenceView) throws XavaException {
		if (metaViewsReferences == null)
			metaViewsReferences = new HashMap();
		else {
			if (metaViewsReferences.containsKey(metaReferenceView.getReferenceName())) {
				throw new XavaException("reference_view_already_defined", metaReferenceView.getReferenceName(),
	...
	public void addMetaViewProperty(MetaPropertyView metaPropertyView) throws XavaException {
		if (metaViewsProperties == null)
			metaViewsProperties = new HashMap();
		else {
			if (metaViewsProperties.containsKey(metaPropertyView.getPropertyName())) {
				throw new XavaException("property_view_already_defined", metaPropertyView.getPropertyName(), getName(),
						getModelName());
			}
		}
	...
	public void addMetaViewCollection(MetaCollectionView metaCollectionView) throws XavaException {
		if (metaViewsCollections == null)
			metaViewsCollections = new HashMap();
		else {
			if (metaViewsCollections.containsKey(metaCollectionView.getCollectionName())) {
				throw new XavaException("collection_view_already_defined", metaCollectionView.getCollectionName(),
						getName(), getModelName());
			}
		}
	...
	private MetaMember modify(MetaMember member) throws XavaException {
		...
		String idLabel = isSection() ? getParent().getId() + "." + member.getName() : getId() + "." + member.getName();
	...
	/**
	 * If {@code newName} is null or empty is replaced by {@code emptySection}.
	 * <p>
	...
	public void setName(String newName) {
		if (parent != null) { // is section
			if (Is.emptyString(newName)) {
				...
				log.warn(XavaResources.getString("section_name_not_allowed", "is empty", newName));
	...
	private void copyMembersFromExtendedView() {
		if (extendedFromExtendsView || Is.emptyString(getExtendsView()))
			return;
	...
	private Map<String, MetaGroup> cloneMetaGroups(Map<String, MetaGroup> source) {
		if (source == null)
			return null;
		return source.values().stream().map(MetaGroup::cloneMetaGroup)
	...
	private MetaView getMetaExtendsView() {
		...
		if ("DEFAULT".equals(view))
			return getMetaModel().getMetaViewByDefault();
		if (view.startsWith("super.")) {
			MetaModel parent = getParentMetaModel();
			if ("super.DEFAULT".equals(view))
				return parent.getMetaViewByDefault();
	...
	private void setMembersNames(String membersNames, boolean resetSection) throws XavaException {
		...
		if (membersNames.trim().equals("*")) {
			...
			setAllMembers(false);
			if (membersNames.trim().startsWith(";"))
	...
	public IOnChangePropertyAction createOnChangeSearchAction(String refName) throws XavaException {
		...
		if (metaReferenceView != null && !Is.emptyString(metaReferenceView.getOnChangeSearchActionClassName())) {
	...
	/**
	 * To search the objects of type that is represents by this view. <b>Not for the
	 * references</b>
	 */
	public MetaSearchAction getMetaSearchAction() {
		if (!hasMetaSearchAction())
			return null;
	...
	private void calculateDefaultValuesForDescriptionsList(MetaReference r, MetaDescriptionsList metaDescriptionsList)
		...
		if (properties.contains("descripcion"))
			metaDescriptionsList.setDescriptionPropertyName("descripcion");
		else if (properties.contains("description"))
			metaDescriptionsList.setDescriptionPropertyName("description");
		else if (properties.contains("nombre"))
			metaDescriptionsList.setDescriptionPropertyName("nombre");
		else if (properties.contains("name"))
			metaDescriptionsList.setDescriptionPropertyName("name");
		else
	...
	private MetaMemberView getMetaMemberViewFor(String qualifiedMemberName) {
		...
		if (result == null)
			result = getMetaReferenceViewFor(qualifiedMemberName);
		if (result == null)
			result = getMetaCollectionView(qualifiedMemberName);
		return result;
	}
	...
	public List getSections() throws XavaException {
		if (metaModel != null)
			copyMembersFromExtendedView(); // We ask for metaModel because getSections() is used from setMetaModel()
	...
	private void promote(MetaView view) throws XavaException {
	...
	public String getId() {
		if (!Is.emptyString(parentName))
			return modelName + ".views." + parentName; // in section case
		return getIdIfNotSection();
	}

	private String getIdIfNotSection() {
		if (hasName())
			return modelName + ".views." + getName();
		else
	...
	public Collection getPropertiesNamesThrowOnChange() {
		if (propertiesNamesThrowOnChange == null) {
			if (metaViewsProperties == null)
				propertiesNamesThrowOnChange = Collections.EMPTY_LIST;
			else {
				...
				while (it.hasNext()) {
					...
					if (propertyView.hasOnChangeAction() && membersNames
							.contains(StringUtils.substringBefore(propertyView.getPropertyName(), "."))) {
	...
	protected Object clone() throws CloneNotSupportedException {
		...
		if (propertiesNamesThrowOnChange != null)
			clon.propertiesNamesThrowOnChange = new ArrayList(propertiesNamesThrowOnChange);
		if (sections != null)
			clon.sections = new ArrayList(sections);
		if (metaGroups != null)
			clon.metaGroups = new HashMap(metaGroups);
		if (_membersNames != null)
			clon._membersNames = new ArrayList(_membersNames);
		if (metaViewsReferences != null)
			clon.metaViewsReferences = new HashMap(metaViewsReferences);
		if (metaViewsProperties != null)
			clon.metaViewsProperties = new HashMap(metaViewsProperties);
		if (metaViewsCollections != null)
			clon.metaViewsCollections = new HashMap(metaViewsCollections);
		if (metaViewProperties != null)
			clon.metaViewProperties = new HashMap(metaViewProperties);
		if (metaSearchAction != null)
			clon.metaSearchAction = (MetaSearchAction) metaSearchAction.clone();
	...
	public int getLabelFormatFor(MetaMember m) {
		if (m instanceof MetaProperty)
			return getLabelFormatForProperty((MetaProperty) m);
		if (m instanceof MetaReference)
			return getLabelFormatForReference((MetaReference) m);
		return XavaPreferences.getInstance().getDefaultLabelFormat();
	}
	...
	public boolean isAlignedByColumns() {
		if (!isFrame() && getParent() != null)
			return getParent().isAlignedByColumns();
			...
			for (Iterator it = getMetaMembers().iterator(); it.hasNext();) {
				...
				if (member instanceof MetaViewAction) {
					...
					if (!action.isAlwaysEnabled()) {
						if (notAlwaysEnabledViewActionsNames == null)
							notAlwaysEnabledViewActionsNames = new ArrayList();
		...
		for (MetaMember m : allMetaMembers) {
			if ((m.getMetaModel() != null) && !(m.getName().equalsIgnoreCase(PropertiesSeparator.INSTANCE.getName()))) {
				if (Collections.frequency(allMetaMembers, m) != 1 && !duplicated.contains(m.getName())) {
					duplicated = duplicated + m.getName() + " ";
				}
			}
		}
