...
public class ConfigureImportAction extends TabBaseAction implements INavigationAction {
		...
		try {
			...
			if (customFileItem != null && !Is.emptyString(customFileItem.getFileName())) {
				...
				if (fileName.endsWith(".xlsx")) {
					if (!configureImport(excelToCSV(new XSSFWorkbook(inputStream))))
						cancel();
					return;
					...
					if (!configureImport(customFileItem.getString().trim()))
						cancel();
					return;
				} else if (fileName.endsWith(".xls")) {
					if (!configureImport(excelToCSV(new HSSFWorkbook(inputStream))))
						cancel();
					return;
				} else {
	...
	private String formatNumber(double number) {
		if (number % 1 == 0)
			return Long.toString(new Double(number).longValue());
		...
		if (metaProperties == null) {
			...
			for (MetaReference ref : metaModel.getMetaReferences()) {
				for (MetaProperty key : ref.getMetaModelReferenced().getAllMetaPropertiesKey()) {
					...
					qualifiedKey.setLabel(Strings.firstUpper(
							(Labels.get(key.getName()) + " " + XavaResources.getString("of") + " " + ref.getLabel())
									.toLowerCase()));
	...
	private Collection<ImportColumn> setProperties(Collection<MetaProperty> properties,
			Collection<ImportColumn> columns, int distance) {
	...
	private MetaProperty getPropertyWithDistance(Collection<MetaProperty> properties, String nameInFile, int distance) {
		for (MetaProperty property : properties) {
			if (StringUtils.getLevenshteinDistance(property.getLabel(), nameInFile) == distance)
				return property;
		}
		return null;
	}
	...
	private void fillDataSampleLine(Scanner scanner, List<ImportColumn> columns, int lineNumber) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (lineNumber == 1)
				columns.get(i).setSampleContent1(field);
			else
				columns.get(i).setSampleContent2(field); // line 2
		}
	}
