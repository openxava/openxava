...
- **Buscador**: icono `mdi-magnify` dentro de `#search_modules` (añadido en `leftMenu.jsp`). Input pill con fondo `--search-modules-input-background`, `border-radius: var(--radius-full)`, padding vertical `--space-3` (~44px altura), border visible sin foco (`var(--frame-border)`), y focus ring sutil (`border-color: var(--accent-color)` + `box-shadow: 0 0 0 2px var(--focus-ring-color)` al 20%). `search.png` ya no se usa en `search_modules_text` (sigue en `#xava_search_columns_text`).
...
- **Temas actualizados**:
  ...
  - `dark-overrides.css`: `--modules-list-selected-color: var(--accent-color)`, `--modules-list-selected-background: var(--accent-soft)`, `--search-modules-input-background: color-mix(in srgb, var(--color) 5%, transparent)`. Eliminado `--chat-panel-shadow`.
- **Focus ring global**: `--focus-ring-color` reducido de 45% a 20% de opacidad para una indicación de foco más sutil y moderna.
- **Scrollbar del menú**: `scrollbar-width: thin` + `scrollbar-color` para Firefox; `::-webkit-scrollbar` fino (8px) con `border-radius: var(--radius-full)` para Chromium.
...
| `changelog.txt` | 8 entradas añadidas para el Bloque 2a |
...
Bloque 2a **concluido**. Tests pasados, estética revisada en los 3 temas (Auto/Light/Dark) y modo phone. Changelog actualizado.
